...
//						        "0123456789012345"
const char display0[] PROGMEM = "SK-0000";
...
const char display42[] PROGMEM = "Servo Down      ";   
const char display43[] PROGMEM = "Servo Up        ";    
const char display44[] PROGMEM = "Servo Settings  ";
const char display45[] PROGMEM = "";
...
const char* const myDISPLAY[] PROGMEM = { display0, display1, display2, display3, display4, display5,
											...
											display36, display37, display38, display39, display40};
...
uint8_t menuLCD[MAX_MAIN_MENU][MAX_SUB_MENU] = { {1,2,3,5,0}, {6,7,8,0,0}, {13,15,29,34,0}, {27,28,39,16,0}, {10,11,12,0,0} };
...
uint32_t ticks = 0;		// used for keeping track of relative time, 1 tick = 10 msecs
...
void setup() {
	...
	Timer1.initialize(10000);				// this will give us a 10msec timer interrupt
	Timer1.attachInterrupt(Timer10ms);		// this is the name of the interrupt that will be triggered every 10 msec
...
void loop() {
	...
	if (myStepperDrive.distanceToGo() != 0) {	// this will run every time through this loop()...
		...
		if (times100 >= 100) {				// ***** 1000 msec loop *****
			times100 = 0;					//  *************************
...
void monitorCycleTest(void) {
	...
	if (myLCD.row1_index == INDEX_CYCLE_ACTIVE) {
		switch (myCycleTest.index) {
			...
			case 1:				// move to the bottom position - assume that the motor is at the top position to start
				moveTo(myDriveSettings.bot_position);
				ticks = 0;
				myCycleTest.index = 2;
				break;
			case 2:				// wait for the motor to reach the bottom position
				if (myStepperDrive.currentPosition() == myDriveSettings.bot_position) {
					myCycleTest.index = 3;
					myCycleTest.move_time = (time_ms/1000);
					ticks = 0;
				}
				else if (time_ms >= (uint32_t(myCycleTest.move_timeout)*1000)) {
					// halt the cycle? and report a movement timeout
					
				}
				break;
			case 3:				// wait for the dwell time to expire, then increment the index
			...
			case 4:
				...
				myCycleTest.index = 5;
				break;
			case 5:				// wait for the motor to reach the bottom position
				...
				break;
			case 6:				// wait for dwell time to expire, update the cycle count status on the display, then check if we reached the number of cycles, if not set index = 1 (starting a new cycle)
				if (time_ms >= (uint32_t(myCycleTest.dwell_time) * 1000)) {
					ticks = 0;
					myCycleTest.index = 1;
					clearDwellTimer();
